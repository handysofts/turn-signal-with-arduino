...
byte rightArrowAnimation[arrowMarqueStepsCount][8] = {
  {B00000000, B00000000, B00000000, B10000000, B10000000, B00000000, B00000000, B00000000},
...
void loop() {
  if (isTurningLeft)
    ...
    displaySymbol(rightArrowAnimation);
  else
    clearDisplayFor(ledCtrl);

  // button precessed
  if (digitalRead(leftTurnButtonPin) == HIGH) {
    Serial.println("+++++++++++++++++++++++++++++++++++++++++left");
    if (isTurningLeft) {
      clearDisplayFor(ledCtrl);
      isTurningLeft = false;
  ...
  else if (digitalRead(rightTurnButtonPin) == HIGH) {
...
void displaySymbol(byte symbols[arrowMarqueStepsCount][8]) {
  for (int s = 0; s < arrowMarqueStepsCount; s++) {
    ...
    if (digitalRead(leftTurnButtonPin) == HIGH || digitalRead(rightTurnButtonPin) == HIGH) {
      delay(1);
      break;
    }
  }
